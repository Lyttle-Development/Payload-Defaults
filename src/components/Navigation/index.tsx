...
const Navigation: React.FC = () => {
    ...
    const collections = (config?.collections ?? []).filter(
        (c) => (CONSTANTS?.MODULE_ENABLED && c?.slug in CONSTANTS?.MODULE_ENABLED && CONSTANTS?.MODULE_ENABLED[c?.slug as keyof ModuleEnabled]) ?? false,
    );
    const globals = (config?.globals ?? []).filter(
        (g) => (CONSTANTS?.GLOBAL_ENABLED && g?.slug in CONSTANTS?.GLOBAL_ENABLED && CONSTANTS?.GLOBAL_ENABLED[g?.slug as keyof GlobalEnabled]) ?? false,
    );
